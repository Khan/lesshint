...
function parseColor(colorStr) {
    if (/^rgba/.test(colorStr)) {
        var rgba = colorStr.slice(5, -1).split(",").map(function(item) {
            return +item;
        });
    ...
    } else if (/^rgb/.test(colorStr)) {
        var rgb = colorStr.slice(4, -1).split(",").map(function(item) {
            return +item;
        });
        ...
        if (colorStr.length === 3) {
            var rgb = colorStr.split("").map(function(component) {
...
function findClosestColor(inlineColor, storedColors) {
    var colorObj = parseColor(inlineColor);
