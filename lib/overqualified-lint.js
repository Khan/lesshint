...
const OVERQUALIFIED_REGEX = /(^|[^\w#\.])\w+[#\.][\w\-\_]+/g;
    ...
    cssAST.walkRules(function(rule) {
        ...
        while ((match = OVERQUALIFIED_REGEX.exec(rule.selector)) !== null) {
            ...
            var lessIndex = sourceMap.originalPositionFor({
                ...
                // We add the magic `RegExp.lastIndex` to the column, which
                // represents the index at which to start the next match.
                //
                // This is where the id or class starts
