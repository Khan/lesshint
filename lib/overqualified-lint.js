...
function overqualifiedLint(cssAST, sourceMap, callback) {
    var violations = [];
                ...
                for (var node, i = 0; i < selector.nodes.length; i++) {
                    ...
                    if (hasElement && (hasId || hasClass)) {
                        ...
                        violations.push({
                            ...
                            code: -1,
                            reason: "Overqualified selector (" + part + ")",
                        });

                        // Only report one error per selector part, meaning
                        // div.class.extra will have one error
                        return;
                    }
                }
            });
    ...
    // walkRules accepts a callback to run on each node, but the function is
    // synchronous
